...
class GazeboCarEnv(gymnasium.Env):
    ...
    def reset(self, *, seed=None, options=None):
        ...
        self._teleport("vehicle_blue",x_st, y_st, yaw_st)


        # obs_x, obs_y = self.trajectory.new_rand_pt_obstacle()
        # self._teleport("obstacle_1", obs_x, obs_y, 0.0)

        self.obstacle_names = ["obstacle_1", "obstacle_2", "obstacle_3", "obstacle_4"]
    
        for obs_name in self.obstacle_names:
            # Losuj pozycję tak długo, aż nie będzie zbyt blisko robota
            valid_pos = False
            while not valid_pos:
                x_obs, y_obs = self.trajectory.new_rand_pt_obstacle()
                
                # Sprawdź dystans od robota (żeby nie zespawnować przeszkody NA aucie)
                dist_to_robot = np.sqrt((x_obs - x_st)**2 + (y_obs - y_st)**2)
                if dist_to_robot > 5.0: # min. 5 metrów od startu
                    valid_pos = True
            
            self._teleport(obs_name, x_obs, y_obs, 0.0)
    ...
    def _teleport(self, model_name, x, y, yaw):
